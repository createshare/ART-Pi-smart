...
function dopatch() {
    ...
    patch -Np1 -i ../sdl2-ttf.patch
}

function preparesrc() {
    if [ ! -f ${APP_DIR}.tar.gz ]; then
        echo "========= Downloading ${APP_DIR}.tar.gz ========="
	wget https://www.libsdl.org/projects/SDL_ttf/release/${APP_DIR}.tar.gz
    fi
...
function builddbg() {
    ...
    sed -i 's/\/usr\/include\/freetype2/./g' Makefile
    # disable building showfont
...
rm -rf ${ROOTDIR}/sdk/include/sdl/SDL_ttf.h
rm -rf ${ROOTDIR}/sdk/lib/libSDL2_ttf.a

cp SDL_ttf.h ${ROOTDIR}/sdk/include/sdl
cp .libs/libSDL2_ttf.a ${ROOTDIR}/sdk/lib

echo "========= SDL2-ttf building finished. ========="
