...
if [ ! -f ${LIB_DIR}/libSDL2.a ]; then
        echo "========= Error: not find sdl, please build it first! ========="
        exit 1;
fi
...
function preparesrc() {
	...
	wget https://www.libsdl.org/projects/SDL_net/release/${APP_DIR}.tar.gz
    fi
...
function builddbg() {
	...
	--enable-static
    # remove "/usr/include/SDL2"
    sed -i 's/\/usr\/include\/SDL2/./g' Makefile
    # disable building showinterfaces
    sed -i 's/$(PROGRAMS)//g' Makefile
    make clean
    if [ "$2" == "verbose" ]; then
        make V=1
    else
...
rm -rf ${ROOTDIR}/sdk/include/sdl/SDL_net.h
rm -rf ${ROOTDIR}/sdk/lib/libSDL2_net.a

cp SDL_net.h ${ROOTDIR}/sdk/include/sdl
cp .libs/libSDL2_net.a ${ROOTDIR}/sdk/lib

echo "========= SDL2-net building finished. ========="
