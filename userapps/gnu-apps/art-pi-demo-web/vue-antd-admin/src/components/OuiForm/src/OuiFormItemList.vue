<template>
  <oui-form-item-template ref="form-item" v-bind="OuiFormItemTemplateProps" :rules="listRules">
    <div class="oui-form-list">
      <a-tag v-for="tag in model" :key="tag" closable @close="handleDelDlist(tag)">{{ tag }}</a-tag>
      <template v-if="inputVisible">
        <a-input ref="input" v-model="inputValue" size="small" @change="validate" @blur="handleinputConfirm" @pressEnter="handleinputConfirm"></a-input>
      </template>
      <a-button v-else size="small" type="primary" style="width: 100px" @click="showInput">+ {{ $t('Add') }}</a-button>
    </div>
