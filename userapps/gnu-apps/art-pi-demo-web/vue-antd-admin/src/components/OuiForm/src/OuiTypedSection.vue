  ...
  <component :is="container" :title="title">
    <a-table v-if="columns" :columns="tableColumns" :data-source="dataSource">
      ...
      <template v-for="c in columns" v-slot:[c.name]="text,record">
        <slot :name="c.name" :s="record"></slot>
      </template>
      <template v-for="c in columns.filter(c => c.titleSlot)" v-slot:[c.titleSlot]>
        <slot :name="c.titleSlot"></slot>
      </template>
      <template v-slot:oui-form-table-action="sid">
        <a-button v-if="sortable" size="small" type="primary" style="margin-right: 5px" icon="arrow-up" @click="handleSort(sid, -1)"></a-button>
        <a-button v-if="sortable" size="small" type="primary" style="margin-right: 5px" icon="arrow-down" @click="handleSort(sid, 1)"></a-button>
      ...
      <template v-if="expanded" #expandedRowRender="record">
        <slot name="expandedRowRender" :s="record"></slot>
      </template>
    </a-table>
        ...
        <a-collapse-panel v-for="s in sections" :key="s['.name']">
          ...
          <template #extra>
            <a-tooltip :title="$t('Delete')">
              <a-icon type="delete" style="color: red" @click.prevent="delSection(s['.name'])"></a-icon>
            </a-tooltip>
          </template>
          <slot :s="s"></slot>
        </a-collapse-panel>
    ...
    <template v-else>
      <div v-for="(s, i) in sections" :key="s['.name']">
        ...
        <a-divider v-if="i < sections.length - 1"></a-divider>
      </div>
    </template>
