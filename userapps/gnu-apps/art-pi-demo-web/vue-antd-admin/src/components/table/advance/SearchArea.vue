<template>
  <div ref="root" class="search-area">
    <div ref="selectRoot" class="select-root"></div>
    <div v-for="(col, index) in searchCols" :key="index" class="search-item">
      <div v-if="col.dataType === 'boolean'" :class="['title', {active: col.search.value !== undefined}]">
        ...
        ></a-switch>
        <a-icon v-if="col.search.value !== undefined" class="close" type="close-circle" theme="filled" @click="e => onCloseClick(e, col)"></a-icon>
      </div>
      <div v-else-if="col.dataType === 'time'" :class="['title', {active: col.search.value}]">
        ...
        <slot v-else-if="col.slots && col.slots.title" :name="col.slots.title"></slot>
        <a-time-picker v-model="col.search.value" :format="col.search.format" placeholder="选择时间" class="time-picker" size="small" :get-popup-container="() => $refs.root" @change="(time, timeStr) => onCalendarChange(time, timeStr, col)" @openChange="open => onCalendarOpenChange(open, col)"></a-time-picker>
      </div>
      <div v-else-if="col.dataType === 'date'" :class="['title', {active: col.search.value}]">
        ...
        <slot v-else-if="col.slots && col.slots.title" :name="col.slots.title"></slot>
        <a-date-picker v-model="col.search.value" :format="col.search.format" class="date-picker" size="small" :get-calendar-container="() => $refs.root" @change="onDateChange(col)"></a-date-picker>
      </div>
      <div v-else-if="col.dataType === 'datetime'" class="title datetime active">
        ...
        <slot v-else-if="col.slots && col.slots.title" :name="col.slots.title"></slot>
        <a-date-picker v-model="col.search.value" :format="col.search.format" class="datetime-picker" size="small" show-time :get-calendar-container="() => $refs.root" @change="(date, dateStr) => onCalendarChange(date, dateStr, col)" @openChange="open => onCalendarOpenChange(open, col)"></a-date-picker>
      </div>
      ...
      <div v-else :class="['title', {active: col.search.value}]">
        <a-popover v-model="col.search.visible" placement="bottom" :trigger="['click']" :get-popup-container="() => $refs.root" @visibleChange="onVisibleChange(col, index)">
          ...
          <a-icon v-if="!col.search.value" class="icon-down" type="down"></a-icon>
          ...
          <div slot="title" class="search-overlay">
            <a-input :id="`${searchIdPrefix}${index}`" v-model="col.search.value" :allow-clear="true" size="small" @keyup.esc="onCancel(col)" @keyup.enter="onConfirm(col)"></a-input>
          </div>
        </a-popover>
        <a-icon v-if="col.search.value" class="close" type="close-circle" theme="filled" @click="e => onCloseClick(e, col)"></a-icon>
      </div>
    </div>
  </div>
</template>
