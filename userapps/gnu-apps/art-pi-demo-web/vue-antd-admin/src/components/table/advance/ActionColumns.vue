<template>
  <div ref="root" class="action-columns">
    <a-popover v-model="visible" placement="bottomRight" trigger="click" :get-popup-container="() => $refs.root">
      <div slot="title">
        <a-checkbox :indeterminate="indeterminate" :checked="checkAll" class="check-all" @change="onCheckAllChange"></a-checkbox>列展示
      ...
      <a-list v-for="(col, i) in columns" :key="i" slot="content" style="width: 100%" size="small">
        <a-list-item>
          <a-checkbox v-model="col.visible" @change="e => onCheckChange(e, col)"></a-checkbox>
          ...
          <template slot="actions">
            <a-tooltip title="固定在列头" :mouse-enter-delay="0.5" :get-popup-container="() => $refs.root">
              <a-icon :class="['left', {active: col.fixed === 'left'}]" type="vertical-align-top" @click="fixColumn('left', col)"></a-icon>
            </a-tooltip>
            <a-tooltip title="固定在列尾" :mouse-enter-delay="0.5" :get-popup-container="() => $refs.root">
              <a-icon :class="['right', {active: col.fixed === 'right'}]" type="vertical-align-bottom" @click="fixColumn('right', col)"></a-icon>
            </a-tooltip>
            <a-tooltip title="添加搜索" :mouse-enter-delay="0.5" :get-popup-container="() => $refs.root">
              <a-icon :class="{active: col.searchAble}" type="search" @click="setSearch(col)"></a-icon>
            </a-tooltip>
          </template>
        </a-list-item>
      </a-list>
      <a-icon class="action" type="setting"></a-icon>
    </a-popover>
  </div>
</template>
