    ...
    >
      ...
      <a-tab-pane v-for="page in pageList" :key="page.path">
        <div slot="tab" class="tab" @contextmenu="e => onContextmenu(page.path, e)">
          <a-icon :class="['icon-sync', {'hide': page.path !== active && !page.loading}]" :type="page.loading ? 'loading' : 'sync'" @click="onRefresh(page)"></a-icon>
          <div class="title" @click="onTabClick(page.path)">{{ pageName(page) }}</div>
          <a-icon v-if="!page.unclose" class="icon-close" type="close" @click="onClose(page.path)"></a-icon>
        </div>
      </a-tab-pane>
    </a-tabs>
