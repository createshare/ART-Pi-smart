<template>
  <admin-layout>
    <contextmenu :item-list="menuItemList" :visible.sync="menuVisible" @select="onMenuSelect"></contextmenu>
    <tabs-head
      v-if="multiPage"
      :active="activePage"
      :page-list="pageList"
    ...
    <div :class="['tabs-view-content', layout, pageWidth]" :style="`margin-top: ${multiPage ? -24 : 0}px`">
      <page-toggle-transition :disabled="animate.disabled" :animate="animate.name" :direction="animate.direction">
        <a-keep-alive v-if="multiPage && cachePage" v-model="clearCaches" :exclude-keys="excludeKeys">
          <router-view v-if="!refreshing" ref="tabContent" :key="$route.path"></router-view>
        </a-keep-alive>
        <router-view v-else-if="!refreshing" ref="tabContent"></router-view>
